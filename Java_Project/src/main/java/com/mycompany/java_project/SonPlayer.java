...
public class SonPlayer {
    ...
    //on garde aussi une reference au son gameover sinon on peut pas l'arreter
    private static Clip sonGameover;
    ...
    public static void arreterMusique() {
        ...
        if (sonGameover != null && sonGameover.isRunning()) {// on arrete aussi le gameover  
            sonGameover.stop();
            sonGameover.close();
        }
    }
    ...
    public static void jouer(String cheminFichier) {
        try {
            ...
            AudioInputStream audio = AudioSystem.getAudioInputStream(fichier); 
            sonGameover = AudioSystem.getClip();
            sonGameover.open(audio);
            sonGameover.start();
